...
import { staffProfilesRepo } from "#/repositories/staffProfiles.repo.js";
...
const createUser = async (data) => {
  ...
  Object.keys(staffProfilePayload).forEach((key) => {
    if (staffProfilePayload[key] === undefined) delete staffProfilePayload[key];
  });

  const staffProfile = await staffProfilesRepo.createStaffProfile(staffProfilePayload);
  const safeUser = toSafeUser(user, staffProfile);
...
const updateUser = async (userId, data) => {
  ...
  if (Object.keys(profilePayload).length > 0) {
    updatedProfile = await staffProfilesRepo.updateByUserId(userId, profilePayload);
    if (!updatedProfile) throw new ApiError(StatusCodes.NOT_FOUND, "Staff profile not found");
  } else {
    updatedProfile = await staffProfilesRepo.findByUserId(userId);
  }
...
const deleteUser = async (userId) => {
  ...
  await Promise.all([
    staffProfilesRepo.deleteByUserId(userId),
...
const lockUser = async (userId) => {
  ...
  const safeUser = toSafeUser(updatedUser, staffProfile);
  return { user: safeUser };
};
