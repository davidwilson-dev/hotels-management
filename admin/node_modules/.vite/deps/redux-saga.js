...
import { t as _objectWithoutPropertiesLoose } from "./objectWithoutPropertiesLoose-Cjs36hLC.js";
import { $ as shouldCancel, C as compose, Ct as MULTICAST, D as createSetContextWarning, Dt as TASK, E as createEmptyArray, Et as SELF_CANCELLATION, G as noop, H as logError, I as getLocation, K as once, L as getMetaInfo, M as flatMap, Ot as TASK_CANCEL, R as identity, S as check, St as MATCH, T as createAllStyleChildCallbacks, Tt as SAGA_LOCATION, U as makeIterator, V as kTrue, W as none, X as remove, _ as asyncIteratorSymbol, _t as symbol$1, a as FLUSH, bt as CHANNEL_END_TYPE, c as JOIN, ct as buffer, d as SET_CONTEXT, dt as iterator, et as shouldComplete, f as TAKE, ft as notUndef, g as assignWithSymbols, gt as stringableFunc, ht as string$1, i as CANCELLED$1, j as expanding, k as detach, kt as TERMINATE, l as RACE, lt as channel$1, mt as promise, n as CALL, o as FORK, ot as wrapSagaDispatch, pt as object, r as CANCEL$1, s as GET_CONTEXT, st as array$1, t as ACTION_CHANNEL, tt as shouldTerminate, u as SELECT, ut as func, v as buffers, vt as undef, wt as SAGA_ACTION, xt as IO, yt as CANCEL, z as internalErr } from "./io-e3db6b7a.development.esm-ifoYVsZ3.js";
...
function multicastChannel() {
	...
	return _ref = {}, _ref[MULTICAST] = true, _ref.put = function put(input) {
		...
		if (matcher === void 0) matcher = wildcard;
		checkForbiddenStates();
		if (closed) {
			cb(END);
			return;
...
function getIteratorMetaInfo(iterator, fn) {
	if (iterator.isSagaIterator) return { name: iterator.meta.name };
...
function runJoinEffect(env, taskOrTasks, cb, _ref9) {
	...
	var joinSingleTask = function joinSingleTask(taskToJoin, cb) {
		...
		} else if (taskToJoin.isAborted()) cb(taskToJoin.error(), true);
		else cb(taskToJoin.result());
	};
...
function newTask(env, mainTask, parentContext, parentEffectId, meta, isRoot, cont) {
	...
	**/
	...
	function end(result, isErr) {
		if (!isErr) {
			if (result === TASK_CANCEL) status = CANCELLED;
...
function proc(env, iterator$2, parentContext, parentEffectId, meta, isRoot, cont) {
	if (iterator$2[asyncIteratorSymbol]) throw new Error("redux-saga doesn't support async generators, please use only regular ones");
	...
	function next(arg, isErr) {
		try {
			...
			if (isErr) {
				...
				**/
				result = func(iterator$2.return) ? iterator$2.return(TASK_CANCEL) : {
					done: true,
					value: TASK_CANCEL
				};
			} else if (shouldTerminate(arg)) result = func(iterator$2.return) ? iterator$2.return() : { done: true };
			else result = iterator$2.next(arg);
			if (!result.done) digestEffect(result.value, parentEffectId, next);
			else {
				/**
				This Generator has ended, terminate the main task and notify the fork queue
				**/
				if (mainTask.status !== CANCELLED) mainTask.status = DONE;
				mainTask.cont(result.value);
			}
		} catch (error) {
			if (mainTask.status === CANCELLED) throw error;
...
function runSaga(_ref, saga) {
	var _ref$channel = _ref.channel, channel = _ref$channel === void 0 ? stdChannel() : _ref$channel, dispatch = _ref.dispatch, getState = _ref.getState, _ref$context = _ref.context, context = _ref$context === void 0 ? {} : _ref$context, sagaMonitor = _ref.sagaMonitor, effectMiddlewares = _ref.effectMiddlewares, _ref$onError = _ref.onError, onError = _ref$onError === void 0 ? logError : _ref$onError;
...
function sagaMiddlewareFactory(_temp) {
	var _ref = _temp === void 0 ? {} : _temp, _ref$context = _ref.context, context = _ref$context === void 0 ? {} : _ref$context, _ref$channel = _ref.channel, channel = _ref$channel === void 0 ? stdChannel() : _ref$channel, sagaMonitor = _ref.sagaMonitor, options = _objectWithoutPropertiesLoose(_ref, _excluded);
	var boundRunSaga;
